...
io.on("connection", (socket) => {
  ...
  socket.on("joinGame", ({ username, game }) => {
    ...
    socket.emit(
      "message",
      `Welcome to TicTacToe, user: ${currentUser.username}, game: ${game.id}`
    );

    //Broadcast a new user joined
    console.log(
      `User: ${currentUser.username} with id: ${currentUser.id} joined to game: ${game.id}`
    );
  ...
  socket.on("message", (message) => {
    ...
    console.log(
      `Message from: ${currentUser.username} with id: ${currentUser.id}, message: ${message}`
    );
  });
  ...
  socket.on("gameMove", (move) => {
    ...
    console.log(
      `Game move from: ${currentUser.username} with id: ${currentUser.id}, move: ${move}`
    );
    ...
    io.to(currentUser.game).emit(
      "message",
      printBoard(currentGame.gameLogic.gameStates)
    );
    ...
    if (currentGame.gameLogic.wonFlag) {
      if (currentGame.gameLogic.winner === "x") {
        io.to(currentUser.game).emit(
          "winnerDeclared",
          `Game won by first player: x.`
        );
      } else if (currentGame.gameLogic.winner === "o") {
        io.to(currentUser.game).emit(
          "winnerDeclared",
          `Game won by second player: o.`
        );
      }
    } else {
  ...
  socket.on("disconnect", () => {
    ...
    if (currentUser) {
      ...
      console.log(
        `User: ${currentUser.username} with id: ${currentUser.id} left!`
      );
      ...
      io.to(currentUser.game).emit(
        "winnerDeclared",
        `Player ${currentUser.username} resigned!`
      );
    }
  });
});
